...
def create_dataset(json_data_filepath, dataset_filepath, encoding, drop_irrelevant_tweets):
    ...
    for df_chunk in pd.read_json(json_data_filepath,
        ...
        # Note: ORDER MATTERS for these Lists during assignment/re-assignment.
        ...
        df_chunk['retweeted_custom'] = df_chunk.apply(compute_retweet, axis=1)
        df_chunk['text_custom'] = \
            df_chunk.apply(compute_full_text, axis=1)
        # df_chunk['language_polyglot'] = \
        #     df_chunk.apply(update_language, axis=1)
        df_chunk[user_fields] = \
            df_chunk.apply(compute_user_series, axis=1)
        df_chunk['hashtags_custom'] = \
            df_chunk.apply(compute_hashtags, axis=1)
        df_chunk['company_custom'] = df_chunk.apply(compute_company, axis=1)

        df_chunk['tweet_url_link_custom'] = df_chunk.apply(compute_url_link, axis=1)
...
def compute_company(row):
    ...
    logger.warning(f"\t\t\tunrecognized company (will be dropped): "
                   f"\n\t\t\t\tid: {row['id']}"
                   f"\n\t\t\t\ttweet: {row['text_custom']}"
                   f"\n\t\t\t\thashtags: {row['hashtags_custom']}")
    return ''
...
if __name__ == '__main__':
    ...
    create_dataset("D:/Dropbox/summer-research-2019/json/dataset_slo_20100101-20180510.json",
                   "D:/Dropbox/summer-research-2019/jupyter-notebooks/attribute-datasets/selected-attributes-full.csv",
                   "utf-8", False)
    end_time = time.time()

    time_elapsed = (end_time - start_time) / 60.0
    print(f"Time taken to process dataset: {time_elapsed} minutes")
